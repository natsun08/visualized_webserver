    ...
    for (;;) {
        ...
        if (strncmp("request", buff, 7) == 0) {
            ...
            if (server_mode == 0) { // Single-threaded
                ...
                add_event(event_id++, n);
                sprintf(buff, "Fibonacci request for %d queued as event %d (Event-driven)", n, event_id - 1);
            }
            ...
            snprintf(buff, sizeof(buff), "Invalid command. Use 'request <number>', 'set mode single', 'set mode multi', 'set mode event', 'metrics', or 'exit'.");
        }
