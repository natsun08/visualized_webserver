  ...
  for (;;) {
    ...
    if (strncmp("fibonacci", buff, 9) == 0) {
      ...
      if (server_mode == 0) { // Single-threaded
        ...
        long long end_ns = get_time_in_nanoseconds();
        long long exec_time_ns = end_ns - start_ns;
        snprintf(buff, sizeof(buff), "Fibonacci request for %d queued (Event-driven) in %lld.", n, exec_time_ns);
      }
	...
	AcceptInput(array, size, n);
        ...
        long long end_ns = get_time_in_nanoseconds();
        long long exec_time_ns = end_ns - start_ns;
        snprintf(buff, sizeof(buff), "Search request for %d queued (Event-driven) in %lld ns.", n, exec_time_ns);
      }
    } 
	...
	int buffer=1 ;
        bzero(buff, MAX);
	IoArgs io_args = {0, "2mb-examplefile-com.txt", buffer};  
	...
	// write(connfd, buff, sizeof(buff));
        ...
        bzero(buff, MAX);
	IoArgs io_args = {0, "2mb-examplefile-com.txt"};  
